...
const surface = "rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6";
const colors = ["bg-primary", "bg-primary/85", "bg-primary/70", "bg-primary/55", "bg-primary/40"];
...
export function CompanyPage() {
  ...
  return <div className="mx-auto max-w-[1440px] space-y-6 pb-6 text-foreground" dir={ar ? "rtl" : "ltr"}>
    <header className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
      <div><p className="mb-2 text-xs font-semibold uppercase tracking-widest text-primary dark:text-blue-300">{copy("WORKFORCE / OVERVIEW", "القوى العاملة / نظرة عامة")}</p>
        <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">{t("company.title")}</h1>
        <p className="mt-2 text-sm text-muted-foreground">{copy("Your people. A clear view.", "موظفوك وأولوياتك ومؤشرات القوى العاملة في مكان واحد.")}</p>
      </div>
      <div className="flex flex-wrap gap-2">
        {actions.map((action, i) => <button key={action.page} onClick={() => setPage(action.page)} className={focus + " inline-flex min-h-11 items-center gap-2 rounded-xl px-4 text-sm font-semibold transition-colors " + (i === 0 ? "bg-primary text-white hover:bg-primary/90" : "border border-border bg-card hover:bg-muted")}>{action.label}<ArrowUpRight aria-hidden="true" className="h-4 w-4 rtl:-scale-x-100" /></button>)}
      </div>
    </header>
    ...
    return <>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map(({label,value,detail,icon: Icon,page}) => <button key={label} onClick={() => setPage(page)} className={focus + " " + surface + " group text-start transition-colors hover:border-blue-400"}>
          <div className="flex items-center justify-between gap-3"><span className="text-sm font-medium text-muted-foreground">{label}</span><span className="rounded-xl bg-blue-50 p-2.5 text-primary dark:bg-blue-950 dark:text-blue-300"><Icon className="h-5 w-5" aria-hidden="true" /></span></div>
      ...
      <section className={surface}>
        ...
        <div className="mt-5 grid gap-3">{attention.map((item) => <button key={item.label} onClick={() => setPage(item.page)} className={focus + " flex min-h-24 items-center gap-4 rounded-xl border border-border p-4 text-start transition-colors hover:bg-muted"}>
        ...
        <Section title={t("company.byDept")} subtitle={copy("Headcount, ranked by department", "عدد الموظفين حسب القسم")}>
          {departments.length ? <ul className="space-y-4">{departments.slice(0, 6).map(([name,value],i) => <li key={name}><div className="mb-2 flex justify-between gap-4 text-sm"><span className="break-words font-medium">{name}</span><span className="shrink-0 tabular-nums">{number(value)} <span className="text-muted-foreground">· {number(o.totalEmployees ? value/o.totalEmployees*100 : 0)}%</span></span></div><div aria-hidden="true" className="h-2 rounded-full bg-muted"><div className={"h-full rounded-full " + colors[i%colors.length]} style={{width: (o.totalEmployees ? value/o.totalEmployees*100 : 0) + "%"}} /></div></li>)}</ul> : empty}
        </Section>
      </div>
      <div className="grid items-start gap-5 xl:grid-cols-[1.25fr_1fr]">
        <Section title={copy("Payroll overview", "نظرة عامة على الرواتب")} subtitle={copy("Recorded payroll totals · latest six months", "إجمالي الرواتب المسجلة · آخر ستة أشهر")}>
          {months.length ? <><div className="h-56 w-full" dir="ltr"><ResponsiveContainer width="100%" height="100%"><AreaChart data={months.map(([month, v]) => ({ month, total: v.total }))} margin={{ top: 12, right: 12, bottom: 0, left: 8 }} accessibilityLayer><CartesianGrid vertical={false} stroke="var(--border)" /><XAxis dataKey="month" tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} axisLine={false} tickLine={false} /><YAxis width={55} tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} tickFormatter={(v: number) => number(v / 1000) + "k"} axisLine={false} tickLine={false} /><Tooltip contentStyle={{ background: "var(--card)", borderColor: "var(--border)", borderRadius: 12, color: "var(--foreground)" }} formatter={(v: number) => [money(v), copy("Payroll", "الرواتب")]} /><Area type="monotone" dataKey="total" stroke="var(--primary)" fill="var(--primary)" fillOpacity={0.08} strokeWidth={2} isAnimationActive={false} /></AreaChart></ResponsiveContainer></div><ul className="sr-only">{months.map(([month, v]) => <li key={month}>{month}: {money(v.total)}</li>)}</ul></> : empty}
        </Section>
