          ...
          <thead>
            <tr className="border-b border-border bg-muted/40">
              ...
              {columns.map((c) => (
                <th
                  key={c.key}
                  className={cn("whitespace-nowrap px-4 py-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground", alignClass(c.align), c.className)}
                >
                  {c.header}
                </th>
              ))}
              {rowActions && <th className="px-4 py-3 text-end text-xs font-semibold uppercase tracking-wide text-muted-foreground">Actions</th>}
            </tr>
          </thead>
          <tbody>
            {pageRows.map((row) => (
